...
extern int evaluacion_doble(int* funcion, float epsilon, int rangos_x, int rangos_y, float* rangos, float* memoria);
...
int procesaEstructura(DatosVFunction_t* datos) {
	...
	if(datos->cantidad_rangos[1] == 0){
		ret = evaluacion_simple(datos->funcionIdentificada, datos->epsilon, datos->cantidad_rangos[0], datos->rangos, datos->puntos);
	}
	else{
		ret = evaluacion_doble(datos->funcionIdentificada, datos->epsilon, datos->cantidad_rangos[0], datos->cantidad_rangos[1], datos->rangos, datos->puntos);
	}
    return ret;
}
