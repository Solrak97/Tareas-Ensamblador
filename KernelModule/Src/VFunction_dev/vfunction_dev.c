...
static ssize_t vfunction_dev_write(struct file *filp, const char __user *buf, size_t len, loff_t *off) {
    ...
    ret = verificaErrores(datos.funcion, datos.epsilon, datos.rangos[0], datos.rangos[1]);
    if(!ret && datos.dosRangos) {
        ret = verificaErrores(datos.funcion, datos.epsilon, datos.rangos[2], datos.rangos[3]);
    }
