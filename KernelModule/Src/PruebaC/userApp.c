...
extern int VFunctionDev_escribe(DatosVFunction_t*);
...
int main() {
    ...
    float* puntos2 = (float*)malloc(sizeof(float) * CANTIDAD_PUNTOS); //12.5MB de puntos
    float* puntos3 = (float*)malloc(sizeof(float) * CANTIDAD_PUNTOS); //12.5MB de puntos
    ...
    //Inicializa los rangos en 0 para todas las funciones
    for(int i = 0; i < 12; ++i) {
        fun1.rangos[i] = 0;
    }
    for(int i = 0; i < 12; ++i) {
    ...
    if(dispositivo >= 0) { //Si hay comunicación con el dispositivo
        ...
        fun2.epsilon = fun1.epsilon;
        fun3.epsilon = fun1.epsilon;
        //FUNCION 1 (EJE X)
        while(!ok && !error) {
